...
async function dispatchOtp(phone, code) {
  ...
  // Dev-only fallback. Never used in production — set OTP_PROVIDER=msg91 or twilio.
  console.log(`[DEV OTP] ${phone} -> ${code} (expires in ${OTP_TTL_MINUTES}m)`);
}
...
export async function verifyOtp(phone, code, purpose = "LOGIN") {
  ...
  if (!otp) return { valid: false, reason: "No OTP request found. Request a new code." };
  if (otp.expiresAt < new Date()) return { valid: false, reason: "OTP expired. Request a new code." };
  if (otp.attempts >= MAX_ATTEMPTS) return { valid: false, reason: "Too many attempts. Request a new code." };
